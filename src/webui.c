    ...
    HRESULT STDMETHODCALLTYPE CreateWebViewControllerHandler_Invoke(
        ...
        if (SUCCEEDED(result) && controller != NULL) {
            ...
            if (_webui.config.show_auto_js_inject) {
                ...
                //     #ifdef WEBUI_LOG
                //     webui_log_debug("[Core]\t\t[Thread .] _webui_webview_thread() -> Auto Inject creation failed\n");
                //     #endif
                // }
                // else {
                //     #ifdef WEBUI_LOG
                //     webui_log_debug("[Core]\t\t[Thread .] _webui_webview_thread() -> Auto Inject creation succeeds\n");
                //     #endif
                // }
            }
        } else return S_FALSE;
